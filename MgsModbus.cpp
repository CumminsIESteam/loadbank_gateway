...
{  
  //****************** Read from socket ****************
  // mjs - Modbus client = Ethernet server???
  ...
  // available() Gets a client that is connected to the server and has data available for reading. The connection persists when the returned client object goes out of scope; you can close it by calling client.stop().
  // MbServer.available() returns a client connected to the server with data available.
  ...
  if(MbsFC == MB_FC_WRITE_REGISTER) {
    Start = word(MbsByteArray[8],MbsByteArray[9]);  // MB offset
    MbData[Start] = word(MbsByteArray[10],MbsByteArray[11]);  // copy message data to server array
    //  response 
    MbsByteArray[5] = 6; //Number of bytes after this one.    // response length feild 
    MessageLength = 12;
    client.write(MbsByteArray, MessageLength);
    MbsFC = MB_FC_NONE;
  }
  ...
  if(MbsFC == MB_FC_WRITE_MULTIPLE_REGISTERS) {
    Start = word(MbsByteArray[8],MbsByteArray[9]); // byte 8-9 = data address of the first register.  (Modbus address = data address + 40001)
    WordDataLength = word(MbsByteArray[10],MbsByteArray[11]); // byte 10-11 = word count
    ByteDataLength = WordDataLength * 2;   
    //MJS added exception response for allowing only writes of 32-bit kw, 32-bit pf, or both   
    if ((Start == 0 && (WordDataLength == 1 || WordDataLength == 2)) || (Start == 2 && WordDataLength == 1))  {
      MbsByteArray[5] = 6;    //  response length (1 (unit ID) + 1 (FC) + 2 (address offset) + 2 (word count) = 6)
      for(int i = 0; i < WordDataLength; i++)
      {
        MbData[Start + i] =  word(MbsByteArray[ 13 + i * 2],MbsByteArray[14 + i * 2]);
      }
    }
    else{     // exception response if addresses are not valid           
      MbsByteArray[5] = 3;    //  response length (1 (unit ID) + 1 (FC) + 1 (exception code) = 3)
      MbsByteArray[7] = 0x90;    // exception function code 
      MbsByteArray[8] = 0;    //  exception code is 0       
    }
    MessageLength = MbsByteArray[5] + 6;
    client.write(MbsByteArray, MessageLength);
    MbsFC = MB_FC_NONE;
  }
  return return_val;
